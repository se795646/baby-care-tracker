...
### 流程 4：智慧清醒預測與發育百分位評估
1. **小睡智慧預測**：當寶寶清醒時，系統自動計算目前已清醒時長。根據寶寶生日（月齡天數）和預設 WHO 建議小睡間隔，結合家長微調的「耐累程度」，自動計算下一次建議小睡的黃金區間，並於倒數計時中主動提醒。
2. **每週更新提醒與換算**：距離上次體重記錄超過 7 天時，系統會自動在首頁彈出溫馨的提醒橫幅。內建「大人抱秤換算助手」供家長快速相減帶入體重。
3. **WHO 發育百分位評估**：每次體重更新後，系統對比 WHO 官方標準發育界線，自動給予家長發育狀態回饋，連動升級首頁每日建議奶量目標。

### 流程 5：語音智慧錄入與口語解析 (Voice Input & Speech NLP)
1. **語音喚起與錄製**：家長在快捷記錄中點選「語音智慧記錄」，啟動內建的 Web Speech API。行動端會連動 `navigator.vibrate` 給予物理輕震回饋。
2. **智慧語音解析**：
   - 語音字串透過 `chineseToNumber` 演算法將口語中文數字（如「一百二十」、「個半」）正規化為阿拉伯數字。
   - 解析器比對口語樣式，提取事件類型（餵奶/睡眠）與對應數量（ml容量/親餵分鐘/睡眠時長）。
3. **即時微調與存檔**：解析結果以卡片顯示，並支持 48px 大按鈕微調，保障行動端單手操作防呆。點選確認後自動換算時間戳記並調用 `saveRecord` 存檔。
...
- **第二階段 (Supabase 雲端整合與即時同步 - 目前階段)：**
  ...
  - 實作首頁「每週提醒更新體重」與「大人抱秤換算助手」，解決體重測量不便痛點。
  - 整合 WHO 嬰兒成長曲線對照發育評估，智慧判斷寶寶發育百分位並提供客觀回饋。
  - 實作智慧清醒追蹤與小睡黃金預測（支援月齡自動切換與家長耐累度客製化偏好微調）。
  - 實作語音智慧記錄（Web Speech API）與自然口語解析器（支援中文數字與時長轉換），並進行行動端專屬優化（物理震動、手機底部抽屜與大按鈕）。
  - 實作行動版「底部導覽列」(Bottom Navigation Bar)，優化手機端單手切換「作息概覽」與「成長曲線」之 RWD 體驗。
  - 重構作息歷史日誌，將完整日誌與搜尋篩選邏輯移至獨立的「歷史日誌」頁面，並在首頁限縮只顯示最近 5 筆記錄，大幅簡化行動端首頁複雜度。
  - 實作「尿布更換記錄」，結合小兒科醫學會黃疸「黃金九色大便卡」與「脫水結晶尿」判定提醒。細部欄位以 JSON 形式存入現有 `note` 欄位，免去調整 Supabase 資料表 Schema 與遷移痛苦。
