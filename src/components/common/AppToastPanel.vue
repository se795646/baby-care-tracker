    ...
    >
        <div class="bq-flex bq-items-start bq-gap-2.5">
            ...
            <div class="bq-text-left">
                <div class="bq-font-bold bq-text-white bq-text-sm">{{ toast.msgTitle }}</div>
                <div v-if="toast.msgContent" class="bq-text-white/80 bq-text-xs bq-mt-0.5">{{ toast.msgContent }}</div>
            </div>
        </div>
        <template #actions>
            <button
                type="button"
                class="bq-text-white hover:bq-text-white/80 bq-text-xs bq-font-bold bq-px-2 bq-py-1"
                @click="closeToast"
            >
                關閉
            </button>
